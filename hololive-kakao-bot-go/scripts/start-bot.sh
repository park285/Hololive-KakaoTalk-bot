...
REQUIRED_VARS="IRIS_BASE_URL HOLODEX_API_KEY_1 REDIS_HOST GEMINI_API_KEY"
...
if ! docker ps | grep "holo-redis" | grep -q "Up"; then
  echo "[WARN] Redis container (holo-redis) is not running!"
  echo "Start it with: docker start holo-redis"
  exit 1
fi

if ! timeout 3 docker exec holo-redis redis-cli ping > /dev/null 2>&1; then
  echo "[WARN] Redis container is running but not responding"
  exit 1
fi
